...
export default async function ExpensesPage({
  ...
  if (params.to) query = query.lte("occurred_at", new Date(`${params.to}T23:59:59`).toISOString());
  if (params.q) {
    const term = `%${params.q}%`;
    query = query.or(`merchant_name.ilike.${term},receiver_name.ilike.${term},note.ilike.${term}`);
  }
      ...
      <div className="grid gap-5">
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-semibold tracking-tight text-ink">Expenses</h1>
          </div>
        ...
        <Card>
          <form className="grid gap-3 sm:grid-cols-5">
            <div className="grid gap-2 sm:col-span-2">
              <Label>Search</Label>
              <Input name="q" defaultValue={params.q} placeholder="Merchant, note, receiver" />
            </div>
